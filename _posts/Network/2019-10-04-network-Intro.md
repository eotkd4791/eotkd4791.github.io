...
- Filtering : 브리지를 못 넘어가게 막는다. 브릿지가 목적지의 맥주소를 알고 있고, 출발지와 목적지가 같은 세그먼트에 있는 경우. 브릿지를 건너가지 않아도 통신이 일어날 수 있다. 따라서 브릿지는 다리를 막는 Filtering을 실시한다. 이 기능 때문에 허브와 다르게 콜리전 도메인을 나누어 줄 수 있다.
