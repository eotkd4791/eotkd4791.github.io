...
- 하나의 호스트에서 다른 호스트로 가는 경로가 두 개 이상 만들어진다면, 루핑이 발생한다.

- 원리 : 한 호스트가 보낸 브로드캐스트 패킷(Broadcast Packet)이 두개의 브릿지로 전달되고 그 두 개의 브릿지는 이 패킷을 분석하고 브릿지가 브로드캐스트 패킷이라는 것을 알아채고 플로딩을 한다. 그럼 상대방 세그먼트로 브로드캐스트 패킷이 보내지고, 그 패킷은 다시 브릿지의 특성에 따라 다시 위로 전달된다. 이런 과정이 반복되면서 한 번 발생한 브로드캐스트 패킷이 양쪽 브릿지를 통해 네트워크를 끝없이 순환하는 루핑이 발생한다. 

- 루핑이 발생하면 CDMA/CD의 특성(한 세그먼트 안에서는 한 호스트만이 통신 가능) 때문에 네트워크에 문제가 발생한다.

- 이러한 루핑을 방지하기 위해서 사람이 네트워크를 잘 설계할 수도 있지만, 한계가 있다. 그래서 루핑을 자동으로 막아주는 스패닝 트리 알고리즘을 이용한다.


## 스패닝 트리 Spanning Tree

- 스패닝 트리 알고리즘은 스위치나 브릿지에서 발생할 수 있는 루핑을 미리 막기 위해 두 개 이상의 경로가 발생하면 하나를 제외하고 나머지 경로를 자동으로 막아두었다가 기존 경로에 문제가 생기면 막아놓은 경로를 풀어서 데이터를 전송한다.

- 전통적인 스패닝 트리 알고리즘은 현재 이용하고 있는 링크가 끊어지고 대기하고 있던 다른 링크를 연결해주는 데에 1분 이상 소요된다. 따라서 사용자들은 그 시간 동안 통신을 못하고 대기해야한다.

- 약점 보완법 -> 이더 채널 : 여러개의 링크가 마치 하나의 링크처럼 인식되게 하는 기술. 업링크 패스트 : 복구 시간을 2~3초 이내에 가능하도록 단축하는 기술.


## 라우터 vs 스위치

- 가격 : 스위치가 더 경제적 (라우터가 비쌈)
- 속도 : 스위치가 빠름 (라우터는 내부에서 처리하는 일이 많다. 따라서 패킷을 처리하는 속도가 느리다.)
- 구성의 편리함 : 스위치가 더 쉬움 (라우터는 라우팅 프로토콜, 네트워크, 보안, 필터링 등.. 설정해야함)


> <b><i>그럼에도 불구하고 라우터를 쓰는 이유</i></b>
>1. 브로드캐스트 영역을 나누기 위해. (안 나누면 .. 해외에서 발생한 브로드 캐스트도 여기까지 전달된다..)
>2. 로드 분배 기능을 제공한다. 한쪽 경로에 문제가 생기면 다른 경로를 이용한다.
>3. 프로토콜이나 데이터 크기, 중요도 등 여러 상황에 따라 트래픽의 전송 순서를 조정해주는 QoS(Quality of Service) 기능 제공.



## IP주소

- TCP/IP 프로토콜을 만들 때 이 프로토콜을 사용하는 모든 장비들을 구분해주기 위해 만들어낸 주소.

- IP주소는 네트워크 부분과 호스트 부분으로 나뉜다. 하나의 네트워크란 라우터 없이도 통신이 가능한 브로드캐스트 영역을 말한다. 호스트 부분은 각각의 PC나 장비를 뜻하는데, 네트워크 영역은 같고 호스트 영역은 달라야 통신이 가능하다. 호스트 부분이 같으면 IP주소 충돌이 생겨서 통신 불능에 빠진다.
